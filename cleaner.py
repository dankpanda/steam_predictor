...
df = pd.read_csv('dataset/merged tag test.csv')
...
'''
'''
df = encode(df,'tags',1000)
df = encode(df,'publisher',1000)
df = encode(df,'developer',1000)
df.drop('tags',axis = 1, inplace=True)
df.drop('publisher',axis = 1, inplace=True)
df.drop('developer',axis = 1,inplace=True)
print(df.shape)'''

calcValue(df, 'popu_tags')
#df.to_csv('dataset/merged tag test.csv',index=False)
